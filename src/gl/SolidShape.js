...
class SolidShape {
  ...
  async init() {
    let gl = Graphics.gl;
    this.solidShader = await Graphics.createShader("solid.vert.glsl", "solid.frag.glsl");
    this.imageShader = await Graphics.createShader("image.vert.glsl", "image.frag.glsl");
    this.imageFlipUv = await Graphics.createShader("image-uvflip.vert.glsl", "image.frag.glsl");
    ...
    this.shapes.rect = {
      size: 6,
      buffer: makeBufferWithData([0, 0, 0, 1, 1, 0, 1, 0, 0, 1, 1, 1]),
    };
  ...
  draw(shape, x, y, w, h, color) {
    let mat = mat4.create();
    mat4.ortho(mat, 0, Graphics.resolution.width, Graphics.resolution.height, 0, -1, 1);
