...
function splitRect(r, seed) {
  ...
  let rnd2 = function (a0, b0, off) {
    ...
    return st.Map(
      st.Lerp(
        random.range(1.0),
        st.Noise(noiseOffset + off + app.kdGridParams.noiseGlobalOffset),
  ...
  static draw(params, a) {
    ...
    for (let x = 0; x < rects.length; x++) {
      ...
      Greeble.fromRect(r.x, r.y, r.w, r.h, colorIntensity).draw(
        app.kdGridParams.blackAndWhite ? 0.0 : app.kdGridParams.spriteChance,
        app
      );

      if (app.kdGridParams.blackAndWhite && app.kdGridParams.drawCircles) {
        let altColor = st.Color(app.getColorFromIntensity(0));
        if (isBlack) {
          console.log(app);
