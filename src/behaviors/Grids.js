  ...
  static drawRandomGrid(p, app) {
    ...
    for (let x = 0; x < p.greebleCount.value * p.scaleCount; x++) {
      getIrregular(
        ...
        vec2(p.minSize, p.maxSize),
        vec2(p.minScaleX, p.minScaleY),
        vec2(p.maxScaleX, p.maxScaleY)
      ).draw(app.randomGridParams.spriteChance, app, function (x) {
        return rgRand.range(x);
      });
    }
  }
