...
import { FieldRepresentation, Toggle } from "./FieldRepresentation.jsx";
...
export class DisplayableComponent extends Component {
  ...
  getParsedParameters() {
    let entries = [];
    entries = Object.entries(this.owner)
  ...
  getElements() {
    return this.getParsedParameters().map((v) => {
      ...
      switch (v.valueType) {
        ...
        case "array":
          ...
          for (let element of v.value) {
            elems.push(
              addElement(DisplayableComponent, {
                key: keyId + i++,
                instance: element,
              })
            );
